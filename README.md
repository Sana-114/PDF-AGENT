...
- 支持显式选择 X/Y/分组字段，按类别计算分组均值，并生成样本标准差、SEM 或正态近似 95% CI 误差棒；
- 同步展示有效样本量、均值、样本标准差、最小值和最大值，样本不足时明确告警而不伪造误差；
- 图表缺失值保留为空缺，超长序列采用等距抽样并明确提示，不把缺失观测伪装成零；
- Figure Caption 直接由最值、首尾变化等可复核统计事实生成，不调用 LLM 推断因果；
- Web 端原生 SVG 图表、数据表预览、归一化说明、独立 SVG 下载及可复制的 300 DPI Matplotlib 脚本；
...
- 正文编号引用与 References 条目双向无损跳转：记录点击来源、从条目一键返回正文，并可循环定位同一文献的上一处/下一处引用；
- 阅读器支持中英文划词/选段翻译，保留公式、代码、引用和数字，零密钥模式明确拒绝伪翻译；
- 支持当前页段落对齐翻译、中英目标语言切换、可开关的双栏双向同步滚动和会话内页级缓存；
...
> 当前版本的语义指纹用于候选预警，不等于最终语义去重模型。当前 OCR 使用 Tesseract 中英文基线，并提供图注引导的有框表格和图区域恢复；复杂合并单元格、旋转表格，以及既无续表标记也无重复表头的跨页表格仍待专用适配器。默认 Embedding 仍是零密钥的确定性 Hash-Ngram 工程基线；真实语义向量和 Cross-Encoder 已提供兼容接口及可选本地编排，但普通启动不会下载模型权重。公式节点是带位置锚点的文本候选，并非可靠的 LaTeX 反演；Docling/GROBID 尚未接入。
...
| POST | `/api/v1/agent/compare` | 对 2–6 篇论文执行均衡检索、来源感知对比与保守冲突识别 |
...
阅读器的精确引用来源选择、正文返回、同标签引用循环和同步滚动开关见 [阅读器导航验收](docs/reader-navigation-2026-09-15.md)。

Transformer v1 的 Table 1/2 无框表格恢复、表格去重和公式 LaTeX 候选指标见 [无框表格与公式候选验收](docs/borderless-table-formula-acceptance-2026-09-16.md)。

两页有框表格在续页无图注时的保守续接、重复表头去重和页级 RAG 锚点见 [跨页表格续接验收](docs/cross-page-table-acceptance-2026-09-17.md)。
...
需要同时启用本地 BGE-M3、Qdrant、BGE Reranker 与 DeepSeek 时运行：
