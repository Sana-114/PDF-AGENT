...
- 抽取有框矢量表格及图注引导的无框表格，输出行、单元格、Markdown、页码与 BBox 锚点；
...
> 当前版本的语义指纹用于候选预警，不等于最终语义去重模型。当前 OCR 使用 Tesseract 中英文基线，并提供图注引导的有框表格和图区域恢复；无框/跨页表格仍待专用适配器。默认 Embedding 仍是零密钥的确定性 Hash-Ngram 工程基线；真实语义向量和 Cross-Encoder 已提供兼容接口及可选本地编排，但普通启动不会下载模型权重。公式节点是带位置锚点的文本候选，并非可靠的 LaTeX 反演；Docling/GROBID 尚未接入。
