...
PaperPilot 是一个以原文证据为核心的科研助手 Agent 系统。本仓库当前为 `0.1.0` 初版框架，已打通 PDF 入库、精确内容去重、异步解析、基础 Document AST、arXiv 版本识别和相似版本提醒。
...
> 当前版本的语义指纹用于候选预警，不等于最终语义去重模型。OCR、Docling/GROBID、向量索引和证据问答将在下一阶段接入。
...
3. 建立 Qdrant 混合检索与 Cross-Encoder 重排；
4. 返回 Claim/Evidence 结构并接入 PDF.js 坐标高亮；
