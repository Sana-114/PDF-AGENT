...
| --- | --- |
| 科研 Agent | 连续问答、多文对比 |
| 知识库 | PDF 上传、解析状态、版本冲突、原文阅读 |
| 论文发现 | 论文检索与导入、订阅与个性化推荐 |
| 研究分析 | 局域引用图谱、证据综述与 Future Work |
| 创作工具 | 写作框架、学术翻译、数据可视化、架构图脚本 |

一级任务区使用固定工作区导航，二级能力只在当前任务区展示。所有旧工作台仍保留既有 URL Hash，页面加载或 Hash 变化时会映射到对应任务区与二级标签，避免界面重组破坏演示链接。桌面端使用侧边任务轨，窄屏改为可横向滚动的任务栏，正文始终保持单一主焦点。
