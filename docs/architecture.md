...
原生文本页先用页面中的学术表格编号提示筛选候选页，再调用较重的矢量网格分析；这避免在 500 页以上文档的纯正文页重复扫描绘图对象。OCR 页使用独立的图注引导栅格恢复：只有出现 `Table N / 表 N` 时才分析横纵线交点并对空单元格局部二次 OCR，只有出现 `Figure N / 图 N` 时才根据相邻墨迹带生成裁剪区域。全页扫描图本身不会被误当成论文插图。无框/跨页表格、无图注图像和复杂合并单元格仍需要专用模型或适配器；公式节点明确标记为 `text_candidate`，在接入公式识别模型前不声称能够无损还原 LaTeX。
